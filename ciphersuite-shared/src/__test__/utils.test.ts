...
describe('Test utility functions', () => {
    ...
    test('string-binary conversion', () => {
        ...
        expect(shouldBeSpecID).toEqual(OPRF_SPEC_ID)
    })
    ...
    test('Context strings', () => {
        ...
        expect(contextString(OPRFMode.Base, OPRFCiphersuite.Ristretto255SHA512, OPRF_SPEC_ID)).toEqual(
            BASE_MODE_CONTEXT_STRING
        )
        expect(contextString(OPRFMode.Verified, OPRFCiphersuite.Ristretto255SHA512, OPRF_SPEC_ID)).toEqual(
            VERIFY_MODE_CONTEXT_STRING
        )
    })
    ...
    test('makeDST', () => {
        ...
        const dst = makeDST('Test-', contextString(OPRFMode.Verified, OPRFCiphersuite.Ristretto255SHA512, OPRF_SPEC_ID))
        const expected = Uint8Array.from([...latin1ToBytes('Test-'), ...VERIFY_MODE_CONTEXT_STRING])
    ...
    test('CT_EQUAL', () => {
        ...
        for (let i = 0; i < numTests; ++i) {
            ...
            for (const suffix of suffixes) {
                ...
                const shouldBeFalse = CT_EQUAL(testArray, baseArray)
        ...
        for (const key of Object.keys(results)) {
            ...
            console.log({
                key,
                stats,
                propVar: ((totalStats.mean - stats.mean) * Math.sqrt(numTests)) / stats.stddev,
            })
